...
# Create a 3-column layout for analysts
col_width = 60
for i in range(0, len(analysts), 3):
    pdf.cell(col_width, 7, '• ' + analysts[i], 0, 0, 'L')
    if i + 1 < len(analysts):
        pdf.cell(col_width, 7, '• ' + analysts[i + 1], 0, 0, 'L')
    if i + 2 < len(analysts):
        pdf.cell(col_width, 7, '• ' + analysts[i + 2], 0, 1, 'L')
    else:
        pdf.ln()
pdf.ln(5)
